...
``date-released``
    ...
    A technote that declares no ``date_updated`` therefore generates a file with no ``date-released`` at all — the Citation File Format requires the field at neither level, so such a file is valid CFF 1.2.0 and is written as undated rather than as dated to a day nobody published on.
