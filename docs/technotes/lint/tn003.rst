:og:description: TN003 fires when a technote's requirements.txt pins Sphinx as a separate requirement instead of relying on the version constrained by documenteer[technote].
...
- :doc:`TN002 <tn002>` — :file:`requirements.txt` must declare ``documenteer[technote]``.
